...
export const makeSelectConnectionsByCaseId = () =>
    createSelector(
        ...
        (connections, caseId) => connections.filter((c) => c.caseId === caseId),
    );
...
export function useConnections() {
    ...
    const renewConnection = (update: Update<Connection, string>) =>
        dispatch(updateConnection(update));

    return { allConnections, pinConnection, unpinConnection, renewConnection };
}

export function useConnectionsForCase(caseId: string) {
    const selectConnections = useMemo(makeSelectConnectionsByCaseId, []);
